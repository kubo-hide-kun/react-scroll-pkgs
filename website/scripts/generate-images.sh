...
for file in ./website/public/assets/first_view_pc/*.jpg
do
  jpegoptim -m90 --strip-all "$file"

  file_name=`get_last_dir $file`
  npm run -w website convert2avif "${target_dir}/${file_name}" -- -d "${target_dir}"
  npm run -w website convert2webp "${target_dir}/${file_name}" -- -d "${target_dir}"
done
